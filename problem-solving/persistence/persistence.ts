 ...
 */
const productOfDigits = (number: number): number => {
  let product = 1;
  let digit = 1;
  while (number > 0) {
    // Time complexity O(m) where m denotes the number of digits
    digit = number % 10;
 ...
 * Time complexity log(n) * m, Space complexity O(1)
 */
const persistence = (number: number): number => {
  let result,
    noOfIterations = 0;
  if (number < 10) {
    noOfIterations = 0;
